...
var _ = Describe("controller", Ordered, func() {
	const runnerNs = "hestia-cron-daemonset"
	const ds1Ns = "hestia-cron-daemonset-1"
	const ds2Ns = "hestia-cron-daemonset-2"

	BeforeAll(func() {
		By("creating namespaces")
		for _, ns := range []string{runnerNs, ds1Ns, ds2Ns} {
	...
	Context("operator", func() {
		It("should watch and schedule job for daemonset", func() {
			By("creating daemonset 1")
			replacements := map[string]interface{}{
				"name":           "cron-daemonset-1",
				"readinessDelay": "1",
				"appLabel":       "cron-daemonset-job",
			}
			utils.ApplyFixtureTemplate("./test/e2e/fixtures/daemonsets/busybox.yaml", ds1Ns, replacements)

			By("creating daemonset 2")
			replacements = map[string]interface{}{
				"name":           "cron-daemonset-2",
				"readinessDelay": "2",
				"appLabel":       "cron-daemonset-job",
			}
			utils.ApplyFixtureTemplate("./test/e2e/fixtures/daemonsets/busybox.yaml", ds2Ns, replacements)

			By("creating scheduled runner")
			replacements = map[string]interface{}{
				"name":            "cron-daemonset-runner",
				"jobDuration":     "1",
				"schedule":        "* * * * *", // every minute
				"deadlineSeconds": 120,
				"appLabel":        "cron-daemonset-job",
			}
			utils.ApplyFixtureTemplate("./test/e2e/fixtures/daemonsets/scheduled_runner.yaml", runnerNs, replacements)
