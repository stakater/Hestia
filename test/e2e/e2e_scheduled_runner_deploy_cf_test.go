...
var _ = Describe("controller", Ordered, func() {
	...
	Context("operator", func() {
		It("should watch and schedule job for deployment-config", func() {
			By("creating deployment-config 1")
			replacements := map[string]interface{}{
				"name":           "dcf-scheduled-1",
				"readinessDelay": "1",
				"appLabel":       "dcf-job",
			}
			...
			replacements = map[string]interface{}{
				"name":            "dcf-cron-runner",
				"jobDuration":     "1",
				"schedule":        "* * * * *", // every minute
				"deadlineSeconds": 120,
				"appLabel":        "dcf-job",
			}
			utils.ApplyFixtureTemplate("./test/e2e/fixtures/deployment_configs/scheduled_runner.yaml", runnerNs, replacements)
